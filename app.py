...
punt_fg = st.sidebar.checkbox("Punt FG%"); punt_ft = st.sidebar.checkbox("Punt FT%"); punt_3pm = st.sidebar.checkbox("Punt 3PM")
punt_reb = st.sidebar.checkbox("Punt REB"); punt_ast = st.sidebar.checkbox("Punt AST"); punt_stl = st.sidebar.checkbox("Punt STL")
punt_blk = st.sidebar.checkbox("Punt BLK"); punt_pts = st.sidebar.checkbox("Punt PTS"); punt_tov = st.sidebar.checkbox("Punt TOV")
...
if st.sidebar.button("אפס דראפט"):
    cursor.execute('DELETE FROM Draft_State'); conn.commit(); st.rerun()
...
query = f'''
WITH PlayerPool AS (SELECT p.Player_ID, p.Full_Name, p.Team, pr.* FROM Players p JOIN Projections pr ON p.Player_ID = pr.Player_ID WHERE pr.MIN > 15 AND pr.Games_Played > 10 AND p.Player_ID NOT IN (SELECT Player_ID FROM Draft_State)),
LeagueAvg AS (SELECT AVG(PTS) as avg_pts, AVG(REB) as avg_reb, AVG(AST) as avg_ast, AVG(STL) as avg_stl, AVG(BLK) as avg_blk, AVG(Three_PM) as avg_3pm, AVG(TOV) as avg_tov, SUM(FG_Made)/SUM(FG_Att) as lg_fg_pct, SUM(FT_Made)/SUM(FT_Att) as lg_ft_pct FROM PlayerPool),
PlayerImpact AS (SELECT pp.*, (pp.FG_Made - (la.lg_fg_pct * pp.FG_Att)) as fg_impact, (pp.FT_Made - (la.lg_ft_pct * pp.FT_Att)) as ft_impact FROM PlayerPool pp CROSS JOIN LeagueAvg la),
...
SELECT pi.Full_Name as Player, pi.Team, 
       ROUND(((pi.PTS - la.avg_pts)/NULLIF(ld.std_pts,0))*{w['pts']} + ((pi.REB - la.avg_reb)/NULLIF(ld.std_reb,0))*{w['reb']} + ((pi.AST - la.avg_ast)/NULLIF(ld.std_ast,0))*{w['ast']} + ((pi.STL - la.avg_stl)/NULLIF(ld.std_stl,0))*{w['stl']} + ((pi.BLK - la.avg_blk)/NULLIF(ld.std_blk,0))*{w['blk']} + ((pi.Three_PM - la.avg_3pm)/NULLIF(ld.std_3pm,0))*{w['3pm']} + (((pi.TOV - la.avg_tov)/NULLIF(ld.std_tov,0))*-1)*{w['tov']} + ((pi.fg_impact - lis.avg_fg_imp)/NULLIF(ld.std_fg,0))*{w['fg']} + ((pi.ft_impact - lis.avg_ft_imp)/NULLIF(ld.std_ft,0))*{w['ft']}, 2) as Total_Value
FROM PlayerImpact pi CROSS JOIN LeagueAvg la CROSS JOIN LeagueImpactStats lis CROSS JOIN LeagueDeviations ld ORDER BY Total_Value DESC LIMIT 100;
'''
df_board = pd.read_sql(query, conn)
st.subheader("📊 Live Big Board"); st.dataframe(df_board, use_container_width=True)
